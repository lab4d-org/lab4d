...
seqname=predictor-fromft-4x-overfit-randrot-randmaskp-longl
logname=$1
diffgs_path=$2
dev=$3
iters_per_round=50
...
bash scripts/train.sh projects/predictor/train.py $dev --seqname $seqname --logname $logname --num_rounds 40 \
  ...
  --diffgs_path $diffgs_path --xyz_wt 0.01 --model_type object

rm -rf logdir/$seqname-$logname-ft1
bash scripts/train.sh projects/predictor/train.py $dev --seqname $seqname --logname $logname-ft1 --num_rounds 40 \
  --load_path logdir/$seqname-$logname/ckpt_latest.pth \
  --imgs_per_gpu 64 --iters_per_round $iters_per_round \
  --diffgs_path $diffgs_path --xyz_wt 0.01 --model_type object

rm -rf logdir/$seqname-$logname-ft2
bash scripts/train.sh projects/predictor/train.py $dev --seqname $seqname --logname $logname-ft2 --num_rounds 40 \
  --load_path logdir/$seqname-$logname/ckpt_latest.pth \
  --imgs_per_gpu 64 --iters_per_round $iters_per_round \
  --diffgs_path $diffgs_path --xyz_wt 0.01 --model_type object

rm -rf logdir/$seqname-$logname-ft3
bash scripts/train.sh projects/predictor/train.py $dev --seqname $seqname --logname $logname-ft3 --num_rounds 40 \
  --load_path logdir/$seqname-$logname/ckpt_latest.pth \
  --imgs_per_gpu 64 --iters_per_round $iters_per_round \
  --diffgs_path $diffgs_path --xyz_wt 0.01 --model_type object
