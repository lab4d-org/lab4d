...
if __name__ == "__main__":
    ...
    # model
    memory_size = past.shape[0]
    forecast_size = x0_wp.shape[0]
    num_kps = x0_joints.shape[1]
    env_model, goal_model, waypoint_model, fullbody_model, angle_model = load_models(
        config, state_size, forecast_size, memory_size, num_kps
    )
    ...
    while True:
        ...
        if len(visualizer.userwp_list) > 1:
            ...
            cam = spline_interp(cam.view(1, -1), 2, interp_size=memory_size)
            cam = (cam.view(-1, 3) - x0_to_world).view(memory_size, 1, 3)
        ...
        if len(visualizer.goal_list) > 0:
            ...
            goal = None
            xyz_grid = None
            reverse_goal, _ = goal_model.reverse_diffusion(
                nsamp,
                num_timesteps,
                noise_scheduler,
                past[None],
        ...
        # waypoint | goal conditioning
        reverse_wp, _ = waypoint_model.reverse_diffusion(
            nsamp,
            num_timesteps,
            noise_scheduler,
            past[None],
        ...
        reverse_wp = reverse_wp.view(-1, nsamp, forecast_size, 1, 3)
        ...
        reverse_joints, _ = fullbody_model.reverse_diffusion(
            nsamp,
            num_timesteps,
            noise_scheduler,
            past_joints[None],
        ...
        reverse_joints = reverse_joints.view(-1, nsamp, forecast_size, num_kps, 3)
        reverse_angles = reverse_angles.view(-1, nsamp, forecast_size, 1, 3)
        ...
        for i, noisy_joints in enumerate(reverse_joints):
            ...
            noisy_joints_dense = spline_interp(
                noisy_joints, forecast_size=forecast_size + 1, num_kps=num_kps
            )
        ...
        reverse_joints_dense = reverse_joints_dense.view(
            reverse_joints_dense.shape[0], nsamp, -1, num_kps, 3
        )
        # spline | angles
        ...
        for i, noisy_angles in enumerate(reverse_angles):
            ...
            sample_angles_dense = spline_interp(
                noisy_angles, forecast_size=forecast_size + 1
            )
        ...
        visualizer.render_trajectory_viser(
            ...
            reverse_joints_dense[-1, 0].view(-1, num_kps, 3),  # joint angles
        ...
        past = accumulated_traj[-memory_size - 1 : -1]
        # update past_joints, past_angles
        past_joints = reverse_joints_dense[-1, 0][-memory_size - 1 : -1]
        ...
        past_angles = matrix_to_axis_angle(
            ...
            @ axis_angle_to_matrix(pred_angles[-memory_size - 1 : -1])
        )
