...
def read_raw(img_path, delta, crop_size, use_full, with_flow=True, crop_mode="minmax", keep_aspect=False):
    ...
    if not is_detected:  # force using full if there is no detection
        use_full = True
    crop2raw = compute_crop_params(mask, crop_size=crop_size, use_full=use_full, crop_mode=crop_mode, keep_aspect=keep_aspect)
