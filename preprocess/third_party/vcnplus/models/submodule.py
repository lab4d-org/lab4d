...
class bfmodule(nn.Module):
    def __init__(self, inplanes, outplanes):
        ...
        self.proj = conv2DBatchNormRelu(
            in_channels=inplanes, k_size=1, n_filters=64, padding=0, stride=1
        )
        self.inplanes = 64
        # Vanilla Residual Blocks
        self.res_block3 = self._make_layer(residualBlock, 64, 1, stride=2)
        ...
        self.pyramid_pooling = pyramidPooling(128, levels=3)
        # Iconvs
        self.upconv6 = conv2DBatchNormRelu(
            in_channels=128, k_size=3, n_filters=64, padding=1, stride=1
        )
        self.upconv5 = conv2DBatchNormRelu(
            in_channels=64, k_size=3, n_filters=32, padding=1, stride=1
        )
        self.upconv4 = conv2DBatchNormRelu(
            in_channels=64, k_size=3, n_filters=32, padding=1, stride=1
        )
        self.upconv3 = conv2DBatchNormRelu(
            in_channels=64, k_size=3, n_filters=32, padding=1, stride=1
        )
        self.iconv5 = conv2DBatchNormRelu(
            in_channels=128, k_size=3, n_filters=64, padding=1, stride=1
        ...
        self.iconv4 = conv2DBatchNormRelu(
            in_channels=96, k_size=3, n_filters=64, padding=1, stride=1
        )
        self.iconv3 = conv2DBatchNormRelu(
            in_channels=96, k_size=3, n_filters=64, padding=1, stride=1
        )
        ...
        self.proj6 = nn.Conv2d(
            128, outplanes, kernel_size=3, stride=1, padding=1, bias=True
        )
        self.proj5 = nn.Conv2d(
            64, outplanes, kernel_size=3, stride=1, padding=1, bias=True
        )
...
def compute_geo_costs(rot, trans, Ex, Kinv, hp0, hp1, tau, Kinv_n=None):
    ...
    mcost1 = sampson_err(
        Kinv.matmul(hp0.permute(0, 2, 1)),
        Kinv_n.matmul(hp1.permute(0, 2, 1)),
        Ex.cuda().permute(0, 2, 1),
    )  # variable K
...
def sampson_err(x1h, x2h, F):
    ...
    algdis = (l1 * x1h).sum(1)
    dis = algdis**2 / (
        1e-9 + l1[:, 0] ** 2 + l1[:, 1] ** 2 + l2[:, 0] ** 2 + l2[:, 1] ** 2
    )
    return dis
...
def get_intrinsics(intr, noise=False):
    ...
    if noise:
        ...
        cyo = cy.clone()
        f = (
            torch.Tensor(np.random.normal(loc=0.0, scale=delta, size=(bs,)))
            .cuda()
            .exp()
            * fo
        )
...
def F_ngransac(hp0, hp1, Ks, rand, unc_occ, iters=1000, cv=False, Kn=None):
    ...
    out_gradients = torch.zeros(
        probs.size()
    )  # gradient tensor (only used during training)
    ...
    for i in range(b):
        ...
        for rott in [(R1, T), (R2, T), (R1, -T), (R2, -T)]:
            if testEss(
                K0, K1, rott[0], rott[1], hp0_cpu[0, ffmask].T, hp1_cpu[i, ffmask].T
            ):
...
def testEss(K0, K1, R, T, p1, p2):
    ...
    testP = cv2.triangulatePoints(
        K0.dot(np.concatenate((np.eye(3), np.zeros((3, 1))), -1)),
